MERGE (d:Debate {date: '2019-01-14', chamber: 'House of Lords'})
MERGE (p0:Party {name: 'Labour'})
MERGE (p1:Party {name: 'Conservative'})
MERGE (p2:Party {name: 'Bishops'})
...
MERGE (s3:Speaker {id: '#DavidLea'})
ON CREATE SET s3.name = 'David Lea', s3.sex = 'M', s3.parliament = 'House of Lords', s3.contributions = 1
MERGE (s3)-[:MEMBER_OF]->(p0)
MERGE (s3)-[:PARTICIPATED_IN]->(d)

MERGE (s4:Speaker {id: '#AlanSmith'})
ON CREATE SET s4.name = 'Alan Smith', s4.sex = 'M', s4.parliament = 'House of Lords', s4.contributions = 1
MERGE (s4)-[:MEMBER_OF]->(p2)
MERGE (s4)-[:PARTICIPATED_IN]->(d)

MERGE (s5:Speaker {id: '#ShaistaSheehan'})
ON CREATE SET s5.name = 'Shaista Sheehan', s5.sex = 'F', s5.parliament = 'House of Lords', s5.contributions = 1
